...
class DB:
    ...
    def get_db(self):
        ...
        db_f = os.path.join(NIMB_tmp, df_f_name)

        log.info(f"Database file {df_f_name} located at: {NIMB_tmp}")
        if os.path.isfile(db_f):
            with open(db_f) as db_f_open:
                db = json.load(db_f_open)
        else:
        	process_order = ('error', 'processed')
            db = dict()
            for typt in ['FS', 'NILEARN', 'DIPY']:
                db[typt] = {}
    ...
    def Update_DB_new_subjects_and_SUBJECTS_DIR(self, NIMB_tmp,
                                                db,
                                                vars_freesurfer,
                                                DEFAULT):
