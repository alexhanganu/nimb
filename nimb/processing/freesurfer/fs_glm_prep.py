...
# -*- coding: utf-8 -*-
"""
This module contains the GLMManager, a class responsible for preparing all
necessary files for a FreeSurfer General Linear Model (GLM) analysis.

It checks subject readiness, unarchives data if necessary, and generates the
required FSGD and contrast matrix files based on the project's configuration.
"""
...
class GLMManager:
    """
    Orchestrates the preparation for a FreeSurfer GLM analysis.
    """
    def __init__(self, all_vars):
        ...
        self.processed_dir = self.fs_vars['NIMB_PROCESSED']
        self.glm_dir = self.project_vars['STATS_PATHS'].get('FS_GLM_dir')
        self.materials_dir = self.project_vars['materials_DIR'][1]
        
        # Input Files
        self.f_ids_path = path.join(self.materials_dir, 'f_ids.json')
        self.grid_file_path = path.join(self.materials_dir, self.project_vars['fname_groups'])
        
        # Components
        self.table_util = Table()
        self.fs_utils = FS_Utils(self.fs_vars.get('FREESURFER_HOME'))

        self._validate_inputs()

    def prepare_glm_files(self):
        """
        Main public method to run the entire preparation process.
        """
        log.info("Starting FreeSurfer GLM preparation...")
        
        ready_subjects_map = self._check_subject_readiness()
        if not ready_subjects_map:
            log.error("No subjects are ready for GLM analysis. Aborting.")
            return

        self.df_ready = self._prepare_dataframe(ready_subjects_map)
        
        self._generate_fsgd_files()
        self._generate_contrast_files()
        
        log.info("GLM preparation complete.")

    def _validate_inputs(self):
        """Ensure all necessary files and directories exist before starting."""
        if not self.glm_dir or not self.grid_file_path:
            raise ValueError("FS_GLM_dir or fname_groups not defined in project config.")
        os.makedirs(self.glm_dir, exist_ok=True)
        if not path.exists(self.grid_file_path):
            raise FileNotFoundError(f"Grid file not found: {self.grid_file_path}")
        if not path.exists(self.f_ids_path):
            raise FileNotFoundError(f"Processed IDs file not found: {self.f_ids_path}")

    def _check_subject_readiness(self):
        """
        Checks which subjects are ready for GLM, unarchiving if necessary.
        Returns a dictionary mapping BIDS ID to the actual ID in SUBJECTS_DIR.
        """
        grid_df = self.table_util.get_df(self.grid_file_path)
        bids_ids_from_grid = grid_df[self.project_vars['id_col']].tolist()
        processed_ids_map = {k: v[DEFAULT.FREESURFER_KEY] for k, v in load_json(self.f_ids_path).items()}
        
        ready_subjects_map = {}
        for bids_id in bids_ids_from_grid:
            fs_id = processed_ids_map.get(bids_id, bids_id).replace('.zip', '')
            
            if self._is_subject_ready(fs_id):
                ready_subjects_map[bids_id] = fs_id
            else: # Try to unarchive
                archive_path = path.join(self.processed_dir, f"{fs_id}.zip")
                if path.exists(archive_path):
                    log.info(f"Unarchiving {archive_path} for GLM analysis...")
                    ZipArchiveManagement(archive_path, path2xtrct=self.subjects_dir).extract_archive()
                    if self._is_subject_ready(fs_id):
                        ready_subjects_map[bids_id] = fs_id
                    else:
                        log.warning(f"Subject {bids_id} failed readiness check even after unarchiving.")
                else:
                    log.warning(f"Subject {bids_id} (FS ID: {fs_id}) is not in SUBJECTS_DIR and no archive was found.")
        return ready_subjects_map
        
    def _is_subject_ready(self, subject_id):
        """Checks if a single subject has all necessary files (qcache)."""
        subject_path = path.join(self.subjects_dir, subject_id)
        if not path.isdir(subject_path):
            return False
        
        # Use FS_Utils to check for qcache completion
        missing_files = self.fs_utils.check_qcache_files(subject_path)
        return not missing_files

    def _prepare_dataframe(self, ready_subjects_map):
        """Creates and saves a DataFrame with only the subjects ready for GLM."""
        grid_df = self.table_util.get_df(self.grid_file_path)
        
        # Filter the original grid to only include ready subjects
        df_ready = grid_df[grid_df[self.project_vars['id_col']].isin(ready_subjects_map.keys())].copy()
        
        # Map BIDS ID to the actual FreeSurfer ID for the FSGD file
        df_ready['fs_id'] = df_ready[self.project_vars['id_col']].map(ready_subjects_map)

        # Save this cleaned grid for reference
        cleaned_grid_path = path.join(self.glm_dir, f"glm_ready_{Path(self.grid_file_path).name}")
        self.table_util.save_df(df_ready, cleaned_grid_path)
        
        log.info(f"{len(df_ready)} subjects are ready for GLM analysis.")
        return df_ready

    def _generate_fsgd_files(self):
        """Generates all necessary FSGD files."""
        log.info("Generating FSGD files...")
        fsgd_dir = path.join(self.glm_dir, 'fsgd')
        os.makedirs(fsgd_dir, exist_ok=True)

        group_col = self.project_vars["group_col"]
        groups = self.df_ready[group_col].unique().tolist()
        variables = self.project_vars["variables_for_glm"]

        # Simple group difference
        fsgd_path = path.join(fsgd_dir, 'group_diff.fsgd')
        self._create_fsgd_file(fsgd_path, groups)
        
        # Analysis with covariates
        for var in variables:
            fsgd_path = path.join(fsgd_dir, f'group_x_{var}.fsgd')
            self._create_fsgd_file(fsgd_path, groups, covariates=[var])

    def _create_fsgd_file(self, file_path, groups, covariates=None):
        """Writes a single FSGD file."""
        ...
        with open(file_path, 'w') as f:
            ...
            for g in groups:
                f.write(f'Class {g}\n')
            if covariates:
                f.write(f'Variables {" ".join(covariates)}\n')
            
            for _, row in self.df_ready.iterrows():
                if row[group_col] in groups:
                    line = f"Input {row['fs_id']} {row[group_col]}"
                    for cov in covariates:
                        line += f" {row[cov]}"
                    f.write(line + '\n')

    def _generate_contrast_files(self):
        """Generates all necessary contrast (.mtx) files."""
        log.info("Generating contrast files...")
        mtx_dir = path.join(self.glm_dir, 'contrasts')
        ...
        contrasts = {
            ...
            'g1.mtx': '1 0',
            'g2.mtx': '0 1',
            # Example for a covariate interaction
            'g1_vs_g2_age_interaction.mtx': '0 0 1 -1' 
        }
        
        for name, matrix in contrasts.items():
            with open(path.join(mtx_dir, name), 'w') as f:
                f.write(matrix)
