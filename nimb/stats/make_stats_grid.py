...
class MakeGrid:
    ...
    def make_grid(self):
        ...
        if file_with_adjusted:
            ...
            self.cp2materials.append(self.grid_file)
    ...
    def check_outliers_nans(self, df):
        ...
        if rows_nan:
            ...
            return self.populate_missing_data(df_outliers_cor)
        else:
            return df
    ...
    def check_outliers(self, df):
        ...
        df_outlier_cor = self.preproc.outliers_get(df2chk,
                                    file2save=self.file2save_out,
                                    change_to_nan=True)
        df_outlier_cor.index.name = self.id_col
        df_outlier_cor[self.group_col] = df_group[self.group_col]
    ...
    def populate_missing_data(self, df):
        ...
        # df, cols_with_nans = self.preproc.nan_rm_cols_if_more(
        #                             df, cols_with_nans,
        #                             threshold=self.thresh_rm_col)
    ...
    def get_base_file(self):
        ...
        for file in ["fname_Outcor", "fname_NaNcor", "fname_eTIVcor"]:
