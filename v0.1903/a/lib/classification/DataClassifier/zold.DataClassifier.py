...
class DataClassifier:
    ...
    def run(self):
        ...
        for _id in main_indexs:
            ...
            count += 1
            self.Classification_dict[sub_id] = {'anat': {'baseline': {}}}

            self.send_to_dictionnary(sub_id, 'baseline', main_idx_value)
        ...
        for attr, value in self.Classification_dict.items():
		    _id = attr
            if len(value['anat']) > 1:
				for tp:
				subj_id = attr+'_'+tp
				...
				method = long
					if Flair true:
				       cmd= f.write('recon-all -i '+t1 file+ ' -FLAIR'+flair_file+' -subjid '+subj_id)
					if not Flair:
				        f.write('recon-all -i '+t1 file+' -subjid '+attr+'_'+tp)

                send_to_running_log(_id, subjid, stage, method)
            else:
					stage = cross_analysis
				method = cross

					if Flair true:
				        f.write('recon-all -i '+t1 file+ ' -FLAIR'+flair_file+' -subjid '+attr)
					if not Flair:
				        f.write('recon-all -i '+t1 file+' -subjid '+attr)


    def make_batch_file(self, cmd)
			    with open(batchfile.sh, 'w') as f:
			        for line in batch_file_header:
						f.write(line+ '\n')
				    f.write('cd some folder'
				    f.write('\n)
        save file to folde (batches)
        save file to folde (batches)
					
def send_to_running_log(self, ):
    maybe dict() that is finally saved to csv
	or csv/excel and data wiill be send to file continuosely
