...
class PerformGLM():
                        ...
                        for thresh in thresholds:
                            ...
                            for contrast in files_for_glm[contrast_type]['mtx']:
                                ...
                                for gd2mtx in files_for_glm[contrast_type]['gd2mtx']:
                                    system('mri_glmfit --y '+mgh_f+' --fsgd '+fsgd_f_unix+' '+gd2mtx+' --glmdir '+glmdir+' --surf fsaverage '+hemi+' --label '+self.SUBJECTS_DIR+'/fsaverage/label/'+hemi+'.aparc.label --C '+self.PATH+'contrasts/'+contrast)
    ...
    def RUN_sim(self, glmdir, contrast_name, hemi, contrast_type, analysis_name, meas, explanation):
        ...
        for direction in sim_direction:
            if meas != 'curv':
                csd_mc_f = self.FREESURFER_HOME+'/average/mult-comp-cor/fsaverage/'+hemi+'/cortex/fwhm'+fwhm[meas][hemi]+'/'+direction+'/th'+str(cache)+'/mc-z.csd'
