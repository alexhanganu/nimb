...
def remote_setup():
    ...
    with open(path.join(cmaindir, 'a', 'var.py'), 'w') as f:
        ...
        f.write(
            "    return cuser, nimb_dir, nimb_scratch_dir, SUBJECTS_DIR, processed_SUBJECTS_DIR, dir_new_subjects, export_FreeSurfer_cmd, source_FreeSurfer_c$\n")
    ...
    if not path.exists(path.join(cmaindir, 'freesurfer', 'MCRv84')):
        ...
        system("./install -mode silent -agreeToLicense yes -destinationFolder " + path.join(cmaindir, 'freesurfer',
                                                                                            'MCRv84'))
