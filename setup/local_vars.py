...
'''
USER-defined variables for the LOCAL processing environment
cadar.compute.canada.ca
'''

supervisor_account = 'def-hanganua' # variable used on compute canada clusters to create the batches
luser              = 'lucaspsy' # username to access the remote computer. Password will be asked by nimb and stored in sqlite
lusers_list        = ['hanganua','hvt','lucaspsy','hiver85'] # if the number of users on the remote computer are more than 1, can be: ['user1','user2',]
...
# PATHS
NIMB_HOME = path.join('/home','projects',supervisor_account,'a') # where the local/remote nimb folder is downloaded
nimb_scratch_dir=path.join('/scratch',luser,'a_tmp') # Default is NIMB_RHOME/tmp path to a temporary folder where all temporary files will be saved. This folder needs to be put in a place that allows quick reading and writing and to delete the files; This folder can get very big. On compute canada it should be put on scratch
...
FREESURFER_HOME = path.join('/home',luser,'projects',supervisor_account,'freesurfer') # path to the freesurfer folder. Default is $NIMB_HOME/freesurfer. If OS is Windows, pipeline will ask the credentials for a remote Linux/Mac.
FS_SUBJECTS_DIR = path.join('/home',luser,'projects',supervisor_account,'fs-subjects') # Default is 'FREESURFER_HOME/subjects', path to the folder where the subjects are being processed by FreeSurfer.
...
cusers_list = lusers_list
cuser = luser
nimb_dir = NIMB_HOME
SUBJECTS_DIR = FS_SUBJECTS_DIR
processed_SUBJECTS_DIR = path.join('/home',luser,'projects',supervisor_account,'processed_subjects') #must be changed to NIMB_HOME/processed_nimb/processed_fs
dir_new_subjects=path.join('/home',luser,'projects',supervisor_account,'new_subjects')  #must be changed to NIMB_RHOME/new_subjects
