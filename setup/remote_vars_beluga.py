...
# PATHS
NIMB_RHOME = path.join('/home',ruser, 'projects',supervisor_account,'a') # where the local/remote nimb folder is downloaded
...
SOURCE_SUBJECTS_DIR = path.join('/home','projects',supervisor_account,'adni','source','mri') # folder that has the raw MRIs of the subjects in archived or non-archived format
SOURCE_BIDS_DIR = '' # path to a folder that, if NOT blank, pipeline will convert SOURCE_SUBJECTS_DIR to BIDS format
PROCESSED_FS_DIR = path.join('/home','projects',supervisor_account,'adni','processed_fs') # path to local or remote folder that has the data processed with FreeSurfer
