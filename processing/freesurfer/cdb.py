...
def keep_files_similar_params(subjid, nimb_dir, NIMB_tmp, t1_ls_f, flair_ls_f, t2_ls_f):
    grouped_by_voxsize = {}
    for file in t1_ls_f:
        print(NIMB_tmp, '        reading MR data for T1 file: '+file)
        ...
        if vox_size:
            ...
            try:
                ...
                print(NIMB_tmp, '        voxel size used: '+str(vox_size_used))
            ...
            t1_ls_f = t1_ls_f[:1]
            vox_size_used = ''
            print(NIMB_tmp, '        voxel size not detected, the first T1 is used')
    if vox_size_used:
        if flair_ls_f != 'none':
            for file in flair_ls_f:
                print(NIMB_tmp, '        reading MR data for FLAIR file: '+file)
                vox_size = get_MR_file_params(subjid, nimb_dir, NIMB_tmp, file)
        ...
        if t2_ls_f != 'none':
            for file in t2_ls_f:
                print(NIMB_tmp, '        reading MR data for T2 file: '+file)
                vox_size = get_MR_file_params(subjid, nimb_dir, NIMB_tmp, file)
    ...
    print(NIMB_tmp, '        files used: '+str(t1_ls_f)+' '+str(flair_ls_f)+'_'+str(t2_ls_f))
    return t1_ls_f, flair_ls_f, t2_ls_f
